...
// API route for AI defect detection
app.post('/api/detect-defects', express.json({limit: '10mb'}), async (req, res) => {
    try {
        ...
        // Get the absolute path to the Python script
        const pythonScriptPath = path.resolve(path.join(__dirname, '../ai/detect.py'));
        
        // Check if Python script exists
        if (!fs.existsSync(pythonScriptPath)) {
            console.error('Python script not found at:', pythonScriptPath);
            return res.status(500).json({ error: 'Defect detection script not found' });
        }
        ...
        pythonProcess.on('close', (code) => {
            if (code !== 0) {
                ...
                return res.status(500).json({ error: `Detection failed with code ${code}` });
            }
...
// API route to save defect annotations
app.post('/api/save-annotation', express.json({limit: '10mb'}), async (req, res) => {
    try {
        const { image, annotation } = req.body;
        if (!image || !annotation) {
            return res.status(400).json({ error: 'Missing image or annotation data' });
        }

        console.log('Saving defect annotation...');
        ...
        let pythonCommand = 'python';
        if (process.platform === 'win32') {
            try { require('child_process').execSync('py --version'); pythonCommand = 'py'; } 
            catch (error) { /* fallback to python */ }
        } else if (process.platform === 'darwin' || process.platform === 'linux') {
            try { require('child_process').execSync('python3 --version'); pythonCommand = 'python3'; } 
            catch (error) { /* fallback to python */ }
        }
        ...
        pythonProcess.on('close', (code) => {
            if (code !== 0) {
                console.error('Python process exited with code', code);
                return res.status(500).json({ error: `Save failed with code ${code}` });
            }
            ...
            try {
                ...
                console.error('Failed to parse Python output:', parseError);
                res.status(500).json({ error: 'Failed to parse save results' });
            }
        });
    } catch (error) {
        console.error('Server error:', error);
